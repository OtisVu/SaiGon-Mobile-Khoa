...
// Instantiate ConnectDB class
$connObj = new ConnectDB();
$conn = $connObj->connect_DB($conn); // Establish database connection

function checkuser($user, $pass, $conn)
{
    $hashedPassword = md5($pass);
    $stmt = $conn->prepare("SELECT MaKhachHang FROM khachhang WHERE SoDienThoai = ? AND MatKhau = ?");
    $stmt->bind_param("ss", $user, $hashedPassword); // Bind parameters
    $stmt->execute(); // Execute the statement
    $stmt->store_result(); // Store result set
    $stmt->bind_result($maKhachHang); // Bind result variable
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../vendor/css-SaiGon_Mobile/SaiGon_Mobile.min.css">
        ...
        <div class="container-login100">
            <div class="wrap-login100">
                <div class="login100-pic js-tilt" data-tilt>
                    <img src="../img/logo_saigon.png" alt="IMG" style="border-radius: 10px;">
                </div>
